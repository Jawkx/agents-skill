...
- base: `epic-<feature>`
- work: `<feature>/work`
- branches: ordered `branches[].branch_name` from repo-root `epic.yml`; the array may be empty before the first generated branch exists
...
- review branches are the durable review and merge units. Meaningful review history lives on branches, not on `work`.
- if a new human-authored commit is found on a review branch, treat that as accidental authoring state: move the commit onto `work` before `generate`, then regenerate branches from `work`.
- only branches named in repo-root `epic.yml` are official branches.
...
- unlocked branches may be rewritten during `generate`; locked branches are immutable.
...
- `feature`, `base`, `work`
- `branches` as an array; use `branches: []` before the first generated branch exists
- ordered `branches[]` entries with `branch_name`, `intent` when official branches exist
...
- `branches` may be empty before the first generated branch exists
...
- Create it as soon as the epic branches exist, even if no official branches have been generated yet.
- It must never be committed on review branches.
...
- After a `generate` action, the selected changes for the selected branch range must be reflected on the generated review branches.
...
- After a full regenerate through the tip branch, the tip branch should differ from `<feature>/work` only by work-only metadata.
- After a targeted regenerate, any remaining non-metadata diff between the tip branch and `<feature>/work` must be explained as intentional leftover future work or ambiguity.
...
- `upstream_base` does not make that branch an official branch and must not be treated as a generate input for branch content.

## Target Resolution

Resolve in order:

1. explicit user target (PR/branch)
2. PR head branch (`gh pr view`) when the user points at a PR
3. likely branch suggestion from spec intent + changed hunks/paths + branch history

If explicit target exists, use it.

If the likely target is weak or there are multiple plausible branches, show the evidence, ask one direct question, and stop writes.

Do not silently place ambiguous work.

Question template:

"I think this belongs on `04-market-ui` because the patch matches that branch intent. If not, should it land on `03-market-api` instead?"
...
- Use branch intent, current branch state, and patch context. Do not assume a file belongs to exactly one branch.
- If ambiguous hunks would affect writes, ask before continuing or leave them on `work` and report them.

## Generate Scope Rules

- Default generate scope: target branch `N` plus unlocked descendants `N+1..tip`.
- Keep ancestors `1..N-1` unchanged unless the user explicitly asks to regenerate a wider unlocked range.
- When earlier branches are locked, start from the first unlocked branch in scope and regenerate remaining unlocked descendants in order.
- Never place `epic.yml` on a review branch.
- Never pull branch content from branches that are not named in the spec.

## Locked Branches

A branch is locked when merged into base:

`git merge-base --is-ancestor <branch> epic-<feature>`

Locked branches are immutable.
